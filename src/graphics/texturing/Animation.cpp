...
Animation::Animation(const std::string &path, int columns, int rows, int speed, bool colorKeyed, uint32_t color)
        : super(TextureManager::loadTextureFromFile(path, colorKeyed, color)), columns(columns), rows(rows) {
    frame = 0;
    real_frame = 0;
    frames = columns * rows;
